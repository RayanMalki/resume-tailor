...
func GetResumeDOCXArtifactHandler(runsSvc *runs.Service, artifactsSvc *artifacts.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		artifact, err := artifactsSvc.GetByRunIDAndType(r.Context(), runID, artifacts.TypeResumeDOCX)
		if err != nil {
			if errors.Is(err, artifacts.ErrArtifactNotFound) {
				writeError(w, http.StatusNotFound, "not ready")
				return
			}
			slog.Error("failed to load resume docx artifact", "run_id", runID.String(), "error", err)
			writeError(w, http.StatusInternalServerError, "internal server error")
			return
		}
		...
		if err != nil {
			slog.Error("failed to decode resume docx artifact", "run_id", runID.String(), "error", err)
			writeError(w, http.StatusInternalServerError, "internal server error")
			return
		}
