...
- User project controls are supported per run: `pinned | auto | exclude`.
- Accepted project explanations are generated by LLM and stored as an artifact.
...
3. OpenAI receives BM25 signals in prompt context and generates:
   - ATS report + change plan
   - `ResumeSpec` JSON (not direct LaTeX)
4. If project controls were provided, OpenAI also generates accepted-project relevance reasons.
5. Backend renderer converts `ResumeSpec` -> final LaTeX using project template style.
6. LaTeX artifact is saved as `resume_latex`.
7. If reasons were generated, they are saved as `project_reasons`.
8. If `RESUME_PDF_ENABLED=1`, worker tries PDF compile via Tectonic and stores `resume_pdf` when successful.
...
- Project controls (`pinned/auto/exclude`) influence project inclusion decisions.
...
- Run creation accepts optional `projectControls` for project selection behavior.
...
- `run_artifacts_items`:
  - `resume_latex`
  - `resume_pdf` (optional)
  - `project_reasons` (LLM explanations for accepted projects)
